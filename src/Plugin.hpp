...
namespace GOTHIC_NAMESPACE 
{
    ...
    int std_print() {
		...
		zSTRING s;
		par->GetParameter(s);
		// print to Union console
        Union::String::Format(s).StdPrintLine();
		// print to stdout
		std::cout << s << std::endl;
	...
	{
#if __DS_DEBUG__
        Union::String::Format("zExternalFixes initialized").StdPrintLine();
        std::cout << "zExternalFixes initialized" << std::endl;

		// Use boguś's https://github.com/bogu9821/BetterDaedalusExternals
		// when they support new API :hihi:
        parser->DefineExternal("stack_size",
                          stack_size,
                          zPAR_TYPE_INT,
                          zPAR_TYPE_VOID);
		#if ENGINE == 4
        parser->DefineExternal("STD_PRINT",
                          std_print,
                          zPAR_TYPE_VOID,
                          zPAR_TYPE_STRING,
                          zPAR_TYPE_VOID);
